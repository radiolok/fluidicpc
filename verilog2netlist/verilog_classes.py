...
class VerilogFile():
    ...
    def create_skidle(self, output_folder="."):
        ...
        for m in self.modules:
            ...
            output_netlist_filename = f'{output_folder}{os.sep}{m.name}.net'
            output_skidl_filename = f'{output_folder}{os.sep}{m.name}_skidl.py'
            c = Circuit()
            c.no_files = True
            local_wires = {}
            local_inputs = {}
            ...
            f = open(output_netlist_filename, "w")
            print(f'save {output_netlist_filename} ')
            f.write(c.generate_netlist())
            f.close()
            
            # Generate skidl file 
            # That can be edited and then generate new netlist
            f2 = open(output_skidl_filename, "w")
            print(f'save {output_skidl_filename} ')
            f2.write(netlist_to_skidl(c.generate_netlist()))
            f2.close()